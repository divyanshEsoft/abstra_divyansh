...
class ProjectMaster(Document):
	...
	def get_nesting_details_item(self):
		...
		def find_bom(item_code, parent_item_code=None, checked_pairs=None):
			...
			row_bom = next(
				(row.bom_no for row in self.sub_assembly_items if row.production_item == item_code and row.bom_no),
				None
			)
			if row_bom:
				return row_bom
			...
			parent_item_code_next = next(
				(row.parent_item_code for row in self.sub_assembly_items if row.production_item == item_code),
				None
			)

			# 3️⃣ If a parent exists, recurse upward
			if parent_item_code_next:
				return find_bom(parent_item_code_next, parent_item_code_next, checked_pairs)
		...
		for item in self.sub_assembly_items:
			...
			bom_no = item.bom_no or find_bom(item.production_item, item.parent_item_code)
			...
			if custom_weight and qty:
				self.append("nesting_item_details", {
					"item_code": item.production_item,
	...
	# 	def find_bom(item_code, checked_items=None):
	# 		"""Recursively find a BOM for an item by checking its parent_item_code chain in sub_assembly_items."""
	# 		if checked_items is None:
	# 			checked_items = set()
	# 		if not item_code or item_code in checked_items:
	# 			return None  # avoid infinite loop

	# 		checked_items.add(item_code)

	# 		# Try to find a direct BOM for this item
	# 		bom_no = frappe.db.get_value("BOM", {"item": item_code, "is_active": 1}, "name")
	# 		if bom_no:
	# 			return bom_no

	# 		# Find the parent_item_code from the sub_assembly_items table of this doc
	...
	# 		if parent_item_code:
	# 			return find_bom(parent_item_code, checked_items)
	...
	# 		bom_no = item.bom_no or find_bom(item.production_item)

	# 		if bom_no:
	# 			bom_creator = frappe.db.get_value("BOM", bom_no, "bom_creator")
	# 			if bom_creator:
	# 				# Find matching row in BOM Creator Item
	# 				row = frappe.db.get_value(
	# 					"BOM Creator Item",
	# 					{
	# 						"parent": bom_creator,
	# 						"item_code": item.production_item,
	# 						"fg_item": item.parent_item_code,  # match subassembly’s parent_item_code to creator’s fg_item
	# 					},
	# 					["custom_blwt", "qty"],
	# 					as_dict=True
	# 				)
	...
	def add_nesting_items(self):
		...
		new_header = self.append("nesting_header", {
		...
		new_header.scrap_weight = round((new_header.sheet_weight or 0) - (total_weight or 0), 3)
		new_header.net_sheet_weight = round((new_header.nesting_qty or 0) * (new_header.sheet_weight or 0), 3)
		new_header.net_sub_assembly_weight = round((new_header.nesting_qty or 0) * (total_weight or 0), 3)
		new_header.net_scrap_weight = round((new_header.nesting_qty or 0) * (new_header.scrap_weight or 0), 3)

		if new_header.sheet_weight:
			new_header.scrap_percentage = round((new_header.scrap_weight / new_header.sheet_weight) * 100, 3)
			new_header.utilization_percentage = round(100 - new_header.scrap_percentage, 3)

		if round((new_header.nesting_qty or 0) * (new_header.sheet_weight or 0), 3)< round((new_header.nesting_qty or 0) * (total_weight or 0), 3):
			frappe.throw(_("Total weight of items cannot be greater than total sheet weight"))
			
		for item in items_to_add:
			new_item = self.append("nesting_items", {
				"item": item.item_code,
				"qty": item.qty,
		...
		avg_scrap_percentage = (total_scrap_percentage / len(self.nesting_header)) if self.nesting_header else 0
