...
frappe.ui.form.on("Project Master Nesting Item Details", {
	qty: function (frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		if (row.qty) {
			const net_qty = row.qty * frm.doc.nesting_qty
			// if (net_qty > row.pending_qty) {
			// 	frappe.msgprint(__("Net Qty cannot be greater than Pending Qty"));
			// 	frappe.model.set_value(cdt, cdn, "qty", 0);
			// 	frappe.model.set_value(cdt, cdn, "net_qty", 0);
			// }
			frappe.model.set_value(cdt, cdn, "net_qty", row.qty * frm.doc.nesting_qty)
		}
	},
	weight: function (frm, cdt, cdn) {
		update_nesting_item_net_weight(frm, cdt, cdn);
	},
	net_qty: function (frm, cdt, cdn) {
		update_nesting_item_net_weight(frm, cdt, cdn);
	},

	nesting_item_details_remove: function (frm) {
		toggle_add_nesting_button(frm);
	},

});
frappe.ui.form.on("Project Master Nesting Header", {});

function update_nesting_net_weight(frm) {
	const net_weight = (frm.doc.nesting_qty || 0) * (frm.doc.sheet_weight || 0);
	frm.set_value("net_weight", net_weight);
}

function update_nesting_item_net_weight(frm, cdt, cdn) {
	const row = locals[cdt][cdn];
	if (row.weight) {
		const net_weight = row.weight * row.net_qty;
...
frappe.ui.form.on("Project Master", {
	...
	before_save(frm) {
		...
		(frm.doc.nesting_item_details || []).forEach(row => {
			if ((row.pending_qty || 0) > 0) {
				pending_rows.push(
					`Row ${row.idx}: ${row.item_code || '(no item)'} → Pending Qty: ${row.pending_qty}`
				);
			}
		});

		if (frm.confirmed_pending_qty) {
			return;
		}
		...
		if (pending_rows.length > 0) {
			...
			frappe.confirm(
				__(
					'The following rows still have pending quantities:<br><br>{0}<br><br><b>Total Pending Qty: {1}</b><br><br>Do you still want to continue saving?',
					[pending_rows.join('<br>')]
				),
				() => {
					frm.confirmed_pending_qty = true;
					frappe.validated = true;
					frm.save();
				},
				() => {
					frappe.show_alert({
						message: __("Save cancelled please clear the pending quantities."),
						indicator: "orange"
					});
				}
			);
	...
	add_nesting_items: function (frm) {
		if (!(frm.doc.net_weight && frm.doc.nesting_no && frm.doc.sheet_name && frm.doc.nesting_qty && frm.doc.sheet_weight)) {
			frappe.msgprint(__("Please fill all the Nesting Details"));
			return;
		}

		frm.dirty();
		frappe.call({
			method: "add_nesting_items",
			freeze: true,
			doc: frm.doc,
			callback: function () {
				frm.refresh()
	...
	add_nesting_details: function (frm) {
		frm.dirty();

		frappe.call({
			method: "get_nesting_details_item",
			freeze: true,
			doc: frm.doc,
			callback: function () {
				refresh_field("nesting_item_details");
				toggle_add_nesting_button(frm);
			},
		});
	},
	nesting_qty: function (frm) {
		update_nesting_net_weight(frm);
	},
	sheet_weight: function (frm) {
		update_nesting_net_weight(frm);
	},
	...
	get_items_for_material_requests(frm, warehouses) {
		frappe.call({
			method: "abstra.abstra.doctype.project_master.project_master.get_items_for_material_requests",
			freeze: true,
			args: {
				doc: frm.doc,
...
frappe.ui.form.on("Project Master Item", {
...
function toggle_add_nesting_button(frm) {
	const has_data = (frm.doc.nesting_item_details || []).length > 0;
	frm.toggle_display('add_nesting_details', !has_data);
}
...
frappe.ui.form.on("Project Master Sub Assembly Item", {
